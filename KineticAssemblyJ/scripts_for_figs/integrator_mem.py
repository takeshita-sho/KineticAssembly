        ...
        with open(filepath, 'r') as file:    
            ...
            if lines[2].strip() == 'AutoForwardDiff()':
                ...
                if current_nmer < 7:
                    ...
                    mem_match = lines[-3].strip().split()
                    if mem_match[-1] == 'MB':
                        memory_usage = float(mem_match[-2])/1000
                    else: 
                        memory_usage = float(mem_match[-2])
                    
                    # Update the maximum memory usage for this integrator and nmer size
                    if current_nmer in data[current_integrator]:
...
plt.ylabel('Max Peak Memory (GB)',size=20)
...
plt.tight_layout()
plt.savefig('integrator_memory_for.png')
plt.show()
